...
from TranslationPatcher import applyPatches, createPatches, distribute, UnsupportedParamException
...
VERSION = 'v2.7.3'
REPOSITORY = r'Ich73/TranslationToolkitLite'
REPOSITORY_FULL = r'Ich73/TranslationToolkit'
...
def checkUpdates():
	try:
		...
		# show message
		printTitleBox()
		print(' '*m + 'A new version of Translation Toolkit Lite is available.')
		print()
...
def printTitleBox():
	...
	print()
	print(' '*m + '╔' + '═'*(w+2) + '╗')
	title('Translation Toolkit Lite ' + VERSION)
...
def menu():
	...
	printInfo('Uses xdelta patches to patch files from the original game.')
	printOption('CP', 'Create Patches')
	printInfo('Creates xdelta patches for all edited game files.')
	...
	printOption('DS', 'Distribute & Send via FTP', 'DSC', 'Distribute & Send to Citra')
	print()
...
def main():
	try:
	...
	except UnsupportedParamException as e:
		# show message
		printTitleBox()
		print(' '*m + 'You need the full version of Translation Toolkit for this workspace.')
		print()
		printCategory('Options')
		printOption('D', 'Download the full version')
		printOption('E', 'Exit the program')
		print()
		print('_'*(w+m+4+m))
		print()
		...
		if script == 'D': webbrowser.open('https://github.com/%s/releases/latest' % REPOSITORY_FULL)
	except Exception:
		import traceback
		traceback.print_exc()
		print()
